...
#+begin_src emacs-lisp
  (setq package-list '(shackle company org ox-reveal which-key diminish magit irony company-irony
                               irony-eldoc rainbow-mode rainbow-delimiters ws-butler impatient-mode
                               tramp flycheck git-commit git-messenger company-prescient no-littering))
...
(if (string-match "/^(.*?(\bvld\b)[^$]*)$/gm" (irony--find-server-executable))
